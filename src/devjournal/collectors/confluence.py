...
class ConfluenceCollector(Collector):
    ...
    def _fetch(self, target_date: date, config: dict) -> list[dict]:
        # Confluence shares Atlassian auth with Jira — pull from parent config.
        jira_cfg = config.get("collectors", {}).get("jira", {})
        domain = config.get("domain") or jira_cfg.get("domain", "")
        email = config.get("email") or jira_cfg.get("email", "")
        token = config.get("api_token") or jira_cfg.get("api_token", "")
