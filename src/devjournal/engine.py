...
def _section_is_empty(content: str, section_id: str) -> bool:
    ...
    if not non_heading:
        return True
    return all(ln.startswith("*") and ln.endswith("*") for ln in non_heading)
...
class Engine:
    ...
    def _collect_all(self, target_date: date) -> list[CollectorResult]:
        ...
        for collector in self._collectors:
            try:
                cfg = get_collector_config(self._config, collector.config_key)
                result = collector.collect(target_date, {**self._config, **cfg})
                raw.append(result)
            except Exception:
    ...
    def _collect_agenda(
        ...
        for collector in self._collectors:
            try:
                cfg = get_collector_config(self._config, collector.config_key)
                result = collector.collect_agenda(target_date, {**self._config, **cfg})
                if result is None:
                    continue
