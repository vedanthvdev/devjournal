...
def test_scoped_config_does_not_leak_other_collectors(tmp_path):
    """A collector should not receive other collectors' credentials."""
    config = _make_config(tmp_path, ["jira", "gitlab"])
    config["collectors"]["jira"]["api_token"] = "jira-secret"
    config["collectors"]["gitlab"]["token"] = "gitlab-secret"
    engine = Engine(config)

    jira_cfg = engine._scoped_config("jira")
    gitlab_cfg = engine._scoped_config("gitlab")

    assert "api_token" in jira_cfg
    assert "token" not in jira_cfg
    assert "collectors" not in jira_cfg

    assert "token" in gitlab_cfg
    assert "api_token" not in gitlab_cfg
    assert "collectors" not in gitlab_cfg

    assert jira_cfg.get("vault_path") == config["vault_path"]
