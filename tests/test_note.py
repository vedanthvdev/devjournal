...
def test_get_carry_forward_skips_empty_to_find_older(tmp_path):
    """If yesterday's note has no carry-forward items, search older notes."""
    vault = tmp_path / "vault"
    daily = vault / "Journal" / "Daily"
    daily.mkdir(parents=True)

    (daily / "2026-04-13.md").write_text(
        "### Carry Forward\n"
    ...
    items = get_carry_forward(str(vault), date(2026, 4, 15))
    assert len(items) == 1
    assert "two days ago" in items[0]
